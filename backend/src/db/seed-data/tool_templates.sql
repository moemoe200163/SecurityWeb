-- Tool templates (is_approved: false = high-risk, requires admin approval)
...
('sql_dump', 'SQLMap Data Dump', 'sqlmap', 'sqlmap -u {url} --batch --dump', '{"url": []}', false),
...
('hydra_ssh', 'Hydra SSH Brute', 'hydra', 'hydra -l {user} -P {password_list} ssh://{target}', '{"user": [], "password_list": ["/usr/share/wordlists/fasttrack.txt"], "target": []}', false),
('hydra_http', 'Hydra HTTP Brute', 'hydra', 'hydra -l {user} -P {password_list} http-post-form://{target}', '{"user": [], "password_list": ["/usr/share/wordlists/fasttrack.txt"], "target": []}', false)
